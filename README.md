nginx-bde-proxy sets up a container running nginx and [docker-gen][1].  docker-gen generates reverse proxy configs for nginx and reloads nginx when containers are started and stopped.
...
Provided your DNS is setup to forward foo.bar.com to a host running
nginx-proxy, the request will be routed to a container with the VIRTUAL_HOST
env var set.

You can also use the environment CSS_SOURCE in order to activate the automatic
insertion of a (or many) CSS in the HTML pages proxied.
...
### Implementing a New CSS Insertion

Add your CSS files in the /extra-css directory, then insert some sub_filter
rules in the nginx.tmpl file:
